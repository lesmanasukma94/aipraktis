---
title: "50 Prompt ChatGPT untuk UMKM yang Bisa Langsung Digunakan"
description: "Kumpulan 50 prompt ChatGPT untuk UMKM yang dapat membantu pemasaran, penjualan, pelayanan pelanggan, pembuatan konten, dan pengembangan bisnis."
pubDate: "2026-06-21"
author: "AI Praktis Indonesia"
category: "ai-chatbot"
tags:
  - AI
  - Belajar AI
  - AI Chatbot
  - ChatGPT
  - UMKM
  - Prompt
  - untuk
featured: false
draft: false
---

## Ringkasan

ChatGPT dapat menjadi salah satu alat AI yang paling bermanfaat bagi pelaku UMKM. Dengan prompt yang tepat, ChatGPT dapat membantu membuat konten, menyusun strategi pemasaran, menjawab pelanggan, hingga membantu pengembangan bisnis.

Dalam artikel ini Anda akan menemukan 50 prompt ChatGPT untuk UMKM yang dapat langsung digunakan dan dimodifikasi sesuai kebutuhan usaha Anda.

## Mengapa UMKM Perlu Menggunakan Prompt yang Baik?

Kualitas jawaban ChatGPT sangat dipengaruhi oleh instruksi yang diberikan.

Prompt yang jelas akan menghasilkan:

* Jawaban lebih relevan
* Ide yang lebih spesifik
* Konten yang lebih siap digunakan
* Waktu kerja yang lebih hemat

Prompt akan lebih efektif jika dipakai bersama tools yang tepat. Lanjutkan ke [AI Tools untuk UMKM](/blog/ai-tools-untuk-umkm/) untuk memilih tools AI yang cocok untuk promosi, chat pelanggan, katalog, konten, laporan, dan follow-up.

## Prompt ChatGPT untuk Ide Bisnis

### Prompt 1

Saya memiliki usaha [jenis usaha]. Berikan 20 ide pengembangan bisnis yang realistis untuk meningkatkan omzet dalam 6 bulan ke depan.

### Prompt 2

Analisis peluang bisnis baru yang masih berhubungan dengan usaha [jenis usaha].

### Prompt 3

Berikan 10 peluang produk tambahan yang dapat dijual oleh bisnis [jenis usaha].

### Prompt 4

Buat analisis SWOT sederhana untuk usaha [jenis usaha].

### Prompt 5

Berikan strategi diferensiasi agar usaha saya lebih unggul dibanding kompetitor.

## Prompt ChatGPT untuk Marketing

### Prompt 6

Buat strategi pemasaran selama 30 hari untuk usaha [jenis usaha].

### Prompt 7

Berikan 30 ide konten Instagram untuk bisnis [jenis usaha].

### Prompt 8

Buat kalender konten media sosial selama satu bulan.

### Prompt 9

Berikan 20 ide promosi yang tidak membutuhkan biaya besar.

### Prompt 10

Buat strategi pemasaran lokal untuk usaha di kota [nama kota].

## Prompt ChatGPT untuk Caption Media Sosial

### Prompt 11

Buat 10 caption Instagram untuk mempromosikan produk [nama produk].

### Prompt 12

Buat caption promosi yang bersifat soft selling.

### Prompt 13

Buat caption yang mengandung storytelling.

### Prompt 14

Buat caption dengan gaya santai dan ramah.

### Prompt 15

Buat caption yang mendorong interaksi pelanggan.

## Prompt ChatGPT untuk Penjualan

### Prompt 16

Buat skrip penjualan untuk produk [nama produk].

### Prompt 17

Buat contoh pesan follow up pelanggan yang belum membeli.

### Prompt 18

Buat strategi meningkatkan conversion rate penjualan.

### Prompt 19

Buat daftar keberatan pelanggan yang umum dan cara mengatasinya.

### Prompt 20

Buat template penawaran produk yang menarik.

## Prompt ChatGPT untuk Customer Service

### Prompt 21

Buat template balasan pelanggan yang bertanya harga.

### Prompt 22

Buat template balasan pelanggan yang komplain.

### Prompt 23

Buat template balasan pelanggan yang meminta diskon.

### Prompt 24

Buat template balasan pelanggan yang menanyakan stok.

### Prompt 25

Buat template layanan pelanggan yang sopan dan profesional.

## Prompt ChatGPT untuk Toko Online

### Prompt 26

Buat deskripsi produk untuk [nama produk].

### Prompt 27

Buat judul produk yang SEO friendly.

### Prompt 28

Buat FAQ produk yang lengkap.

### Prompt 29

Buat keunggulan produk yang mudah dipahami pelanggan.

### Prompt 30

Buat perbandingan produk dengan kompetitor.

## Prompt ChatGPT untuk Konten

### Prompt 31

Berikan 50 ide konten untuk bisnis [jenis usaha].

### Prompt 32

Buat outline artikel blog tentang [topik].

### Prompt 33

Buat ide video pendek TikTok untuk bisnis saya.

### Prompt 34

Buat script video promosi berdurasi 60 detik.

### Prompt 35

Buat ide konten edukasi untuk pelanggan.

## Prompt ChatGPT untuk Produktivitas

### Prompt 36

Buat daftar tugas harian pemilik UMKM yang lebih efisien.

### Prompt 37

Buat SOP sederhana untuk proses penjualan.

### Prompt 38

Buat SOP pelayanan pelanggan.

### Prompt 39

Buat template laporan mingguan usaha.

### Prompt 40

Buat sistem kerja yang lebih produktif untuk usaha kecil.

## Prompt ChatGPT untuk Pengembangan Bisnis

### Prompt 41

Berikan strategi meningkatkan omzet 20% dalam 3 bulan.

### Prompt 42

Berikan ide program loyalitas pelanggan.

### Prompt 43

Buat strategi meningkatkan pembelian ulang pelanggan.

### Prompt 44

Buat strategi referral pelanggan.

### Prompt 45

Buat strategi meningkatkan reputasi bisnis lokal.

## Prompt ChatGPT untuk Analisis

### Prompt 46

Analisis target pasar untuk usaha [jenis usaha].

### Prompt 47

Analisis kompetitor utama di industri saya.

### Prompt 48

Buat profil pelanggan ideal untuk bisnis saya.

### Prompt 49

Berikan rekomendasi channel pemasaran terbaik untuk usaha saya.

### Prompt 50

Buat rencana pengembangan usaha selama 12 bulan.

## Tips Menggunakan Prompt ChatGPT untuk UMKM

Agar hasil lebih baik:

* Sebutkan jenis usaha secara spesifik.
* Jelaskan target pelanggan.
* Sertakan lokasi bisnis jika relevan.
* Jelaskan tujuan yang ingin dicapai.
* Minta hasil dalam format tabel jika diperlukan.

---

# Cara Menggunakan Prompt ChatGPT untuk UMKM

Prompt ChatGPT untuk UMKM akan lebih bermanfaat jika langsung dikaitkan dengan pekerjaan bisnis sehari-hari.

Jangan hanya memakai prompt umum seperti:

```text
Buatkan ide bisnis.
```

Prompt seperti itu terlalu luas.

Untuk UMKM, prompt yang lebih baik harus menyebutkan:

- jenis usaha,
- produk atau jasa,
- target pelanggan,
- masalah yang ingin diselesaikan,
- channel jualan,
- gaya bahasa,
- format output,
- tujuan akhir.

Contoh prompt yang lebih jelas:

```text
Bertindaklah sebagai konsultan bisnis UMKM.

Saya punya usaha:
[jelaskan usaha]

Produk/jasa:
[jelaskan produk atau jasa]

Target pelanggan:
[jelaskan target]

Channel jualan:
[Instagram/WhatsApp/marketplace/toko offline/website]

Masalah utama:
[jelaskan masalah]

Bantu saya membuat:
1. ide promosi
2. caption
3. template WhatsApp
4. ide konten
5. cara follow-up pelanggan
6. checklist yang bisa langsung saya pakai

Gunakan bahasa Indonesia sederhana dan praktis.
```

Dengan konteks seperti ini, hasil ChatGPT akan lebih relevan untuk bisnis kecil.

Baca juga:

- [Cara Menggunakan ChatGPT untuk UMKM](/blog/cara-menggunakan-chatgpt-untuk-umkm/)
- [AI untuk Bisnis](/blog/ai-untuk-bisnis/)
- [Tools AI untuk Bisnis](/blog/tools-ai-untuk-bisnis/)

---

# Kategori Prompt ChatGPT untuk UMKM

Prompt untuk UMKM sebaiknya dibagi berdasarkan kebutuhan bisnis.

UMKM biasanya membutuhkan bantuan untuk:

- promosi,
- konten,
- caption,
- WhatsApp pelanggan,
- ide produk,
- deskripsi produk,
- customer service,
- follow-up,
- laporan sederhana,
- SOP,
- strategi penjualan,
- ide campaign,
- analisis pelanggan.

Berikut kategori prompt yang paling praktis.

---

# Prompt ChatGPT untuk Promosi UMKM

Promosi adalah kebutuhan paling umum untuk UMKM.

ChatGPT bisa membantu membuat ide promosi yang lebih variatif.

Gunakan prompt berikut:

```text
Bertindaklah sebagai marketing strategist untuk UMKM.

Saya memiliki usaha:
[jenis usaha]

Produk utama:
[nama produk]

Target pelanggan:
[target pelanggan]

Harga produk:
[harga]

Channel promosi:
[Instagram/WhatsApp/TikTok/Facebook/marketplace]

Buatkan:
1. 10 ide promosi
2. 5 ide promo bundling
3. 5 ide promo terbatas
4. 5 caption promosi
5. 5 CTA
6. ide visual untuk setiap promosi

Batasan:
Jangan membuat klaim berlebihan.
Gunakan bahasa yang ramah dan mudah dipahami.
```

Contoh penggunaan:

```text
Usaha:
kopi susu rumahan

Target:
mahasiswa dan pekerja kantor sekitar area kampus

Channel:
Instagram dan WhatsApp

Tujuan:
meningkatkan pesanan harian
```

Prompt ini bisa membantu UMKM membuat promosi tanpa harus mulai dari nol.

---

# Prompt ChatGPT untuk Caption Instagram UMKM

Caption Instagram harus menarik, tetapi tetap jelas.

Untuk UMKM, caption sebaiknya tidak terlalu rumit.

Gunakan prompt:

```text
Buatkan caption Instagram untuk UMKM berikut.

Jenis usaha:
[jelaskan usaha]

Produk:
[nama produk]

Target pembeli:
[target]

Manfaat produk:
[manfaat]

Promo jika ada:
[promo]

Gaya bahasa:
ramah, natural, tidak terlalu formal.

Buat:
1. 5 caption pendek
2. 5 caption storytelling
3. 5 caption promosi
4. 5 CTA
5. 5 hashtag relevan
```

Contoh CTA:

```text
Pesan sekarang lewat WhatsApp.
Cek katalog di bio.
Kirim DM untuk tanya stok.
Klik link di bio untuk order.
Simpan postingan ini kalau butuh rekomendasi.
```

Baca juga:

- [Tools AI untuk Instagram](/blog/tools-ai-untuk-instagram/)
- [Prompt AI untuk Social Media](/blog/prompt-ai-untuk-social-media/)

---

# Prompt ChatGPT untuk WhatsApp Pelanggan

Banyak UMKM menjual lewat WhatsApp.

ChatGPT bisa membantu membuat template pesan yang lebih rapi.

Gunakan prompt:

```text
Bertindaklah sebagai admin WhatsApp bisnis.

Saya punya usaha:
[jenis usaha]

Produk/jasa:
[jelaskan]

Buatkan template WhatsApp untuk:
1. menyapa pelanggan baru
2. menjawab pertanyaan harga
3. menjawab pertanyaan stok
4. menjelaskan cara order
5. follow-up pelanggan yang belum transfer
6. mengucapkan terima kasih setelah pembelian
7. meminta testimoni
8. menawarkan repeat order

Gaya bahasa:
ramah, sopan, tidak memaksa.
```

Contoh template follow-up:

```text
Halo Kak, izin follow-up ya.

Kemarin Kakak sempat tanya tentang [produk]. Saat ini stok masih tersedia. Kalau Kakak ingin order, saya bisa bantu proses sekarang.

Terima kasih.
```

Template seperti ini membantu admin membalas lebih cepat dan konsisten.

---

# Prompt ChatGPT untuk Customer Service UMKM

Pertanyaan pelanggan sering berulang.

Misalnya:

- harga berapa,
- stok ada,
- ukuran apa saja,
- bisa kirim ke mana,
- ongkir berapa,
- cara order bagaimana,
- garansi ada atau tidak,
- estimasi pengiriman kapan.

Gunakan prompt:

```text
Bertindaklah sebagai customer service manager untuk UMKM.

Jenis usaha:
[jelaskan]

Produk/jasa:
[jelaskan]

Pertanyaan pelanggan yang sering muncul:
[daftar pertanyaan]

Buat:
1. FAQ pelanggan
2. template jawaban singkat
3. template jawaban ramah
4. template jawaban untuk komplain
5. template eskalasi ke owner/admin
6. SOP sederhana membalas pelanggan

Gunakan bahasa Indonesia yang sopan dan mudah dipahami.
```

Baca juga:

- [AI untuk Customer Service](/blog/ai-untuk-customer-service/)
- [Tools AI untuk Customer Service](/blog/tools-ai-untuk-customer-service/)
- [50 Prompt ChatGPT untuk Customer Service](/blog/50-prompt-chatgpt-untuk-customer-service/)

---

# Prompt ChatGPT untuk Ide Konten UMKM

Konten UMKM tidak harus selalu jualan.

Konten bisa dipakai untuk edukasi, membangun kepercayaan, menjawab pertanyaan, dan mengingatkan pelanggan.

Gunakan prompt:

```text
Bertindaklah sebagai content strategist untuk UMKM.

Jenis usaha:
[jelaskan]

Produk/jasa:
[jelaskan]

Target pelanggan:
[jelaskan]

Channel:
[Instagram/TikTok/Facebook/WhatsApp]

Buatkan 30 ide konten dengan kategori:
1. edukasi
2. promosi
3. behind the scenes
4. testimoni
5. tips
6. FAQ
7. storytelling
8. soft selling

Format output:
- topik
- hook
- poin utama
- format konten
- CTA
```

Contoh format konten:

```text
foto produk,
carousel,
video pendek,
testimoni,
before-after,
tutorial,
cerita pelanggan,
FAQ,
promo harian.
```

Baca juga:

- [Tools AI untuk Content Creator](/blog/tools-ai-untuk-content-creator/)
- [Tools AI untuk TikTok](/blog/tools-ai-untuk-tiktok/)

---

# Prompt ChatGPT untuk Deskripsi Produk

Deskripsi produk sangat penting untuk marketplace, katalog WhatsApp, website, dan Instagram.

Gunakan prompt:

```text
Buatkan deskripsi produk untuk UMKM.

Nama produk:
[nama produk]

Jenis produk:
[jenis]

Bahan/fitur:
[jelaskan]

Manfaat:
[jelaskan]

Target pembeli:
[target]

Harga:
[harga jika ingin dicantumkan]

Buat:
1. deskripsi pendek
2. deskripsi panjang
3. bullet point manfaat
4. keunggulan produk
5. CTA pembelian
6. versi untuk marketplace
7. versi untuk Instagram

Batasan:
Jangan membuat klaim palsu.
Jangan melebih-lebihkan manfaat.
```

Contoh deskripsi yang baik harus menjawab:

```text
Produk ini apa?
Untuk siapa?
Manfaatnya apa?
Kenapa orang perlu membeli?
Bagaimana cara order?
```

---

# Prompt ChatGPT untuk Nama Produk dan Paket Bundling

UMKM bisa memakai ChatGPT untuk membuat nama produk atau paket promo.

Gunakan prompt:

```text
Bertindaklah sebagai branding strategist.

Saya punya produk:
[jelaskan produk]

Target pembeli:
[jelaskan]

Karakter brand:
[ramah/premium/lucu/minimalis/lokal/modern]

Buat:
1. 20 ide nama produk
2. 10 ide nama paket bundling
3. 10 ide nama promo
4. slogan pendek
5. alasan kenapa nama tersebut cocok

Batasan:
Nama harus mudah diingat, tidak terlalu panjang, dan cocok untuk pasar Indonesia.
```

Contoh paket bundling:

```text
Paket Hemat Harian
Paket Keluarga
Paket Coba Dulu
Paket Best Seller
Paket Lengkap
Paket Weekend
```

---

# Prompt ChatGPT untuk Strategi Penjualan UMKM

Strategi penjualan UMKM harus sederhana dan bisa dijalankan.

Gunakan prompt:

```text
Bertindaklah sebagai konsultan penjualan UMKM.

Jenis usaha:
[jelaskan]

Produk/jasa:
[jelaskan]

Target pelanggan:
[jelaskan]

Channel penjualan:
[offline/WhatsApp/Instagram/marketplace/website]

Masalah penjualan saat ini:
[jelaskan]

Buat:
1. strategi penjualan 30 hari
2. ide promosi mingguan
3. cara meningkatkan repeat order
4. cara follow-up pelanggan
5. cara menaikkan nilai transaksi
6. ide bundling
7. metrik yang perlu dicatat

Gunakan langkah praktis dan realistis.
```

Metrik sederhana yang bisa dicatat:

```text
jumlah chat masuk,
jumlah order,
produk paling laku,
sumber order,
jumlah repeat order,
rata-rata nilai transaksi,
keluhan pelanggan,
konten yang paling banyak respon.
```

---

# Prompt ChatGPT untuk Follow-Up Pelanggan

Follow-up sangat penting untuk UMKM, terutama jika pelanggan sering bertanya tetapi belum membeli.

Gunakan prompt:

```text
Buatkan template follow-up pelanggan untuk situasi berikut.

Jenis usaha:
[jelaskan]

Produk:
[jelaskan]

Situasi pelanggan:
[baru tanya harga / belum transfer / minta diskon / sudah pernah beli / lama tidak order]

Buat:
1. follow-up lembut
2. follow-up dengan manfaat produk
3. follow-up dengan promo
4. follow-up untuk repeat order
5. follow-up setelah pembelian

Gaya bahasa:
ramah, sopan, tidak memaksa.
```

Contoh follow-up repeat order:

```text
Halo Kak, semoga harinya lancar ya.

Mau info, produk [nama produk] sudah tersedia lagi. Kalau Kakak ingin repeat order seperti sebelumnya, saya bisa bantu proseskan.
```

---

# Prompt ChatGPT untuk Komplain Pelanggan

Komplain harus dijawab dengan tenang dan profesional.

Gunakan prompt:

```text
Bertindaklah sebagai customer service profesional.

Saya perlu membalas komplain pelanggan.

Jenis usaha:
[jelaskan]

Masalah pelanggan:
[jelaskan komplain]

Kondisi sebenarnya:
[jelaskan kondisi]

Solusi yang bisa diberikan:
[jelaskan solusi]

Buatkan:
1. balasan yang empatik
2. balasan yang menjelaskan solusi
3. balasan singkat untuk WhatsApp
4. catatan internal untuk admin
5. langkah follow-up

Gaya bahasa:
tenang, sopan, bertanggung jawab.
```

Prinsip membalas komplain:

```text
akui keluhan,
minta maaf jika perlu,
jelaskan solusi,
beri timeline,
follow-up sampai selesai.
```

---

# Prompt ChatGPT untuk Laporan Penjualan UMKM

ChatGPT bisa membantu membaca data penjualan sederhana.

Gunakan prompt:

```text
Bertindaklah sebagai analis bisnis UMKM.

Analisis data penjualan berikut.

Data:
[tempel data]

Kolom:
tanggal, produk, jumlah terjual, harga, total penjualan, channel, catatan

Buat:
1. ringkasan penjualan
2. produk paling laku
3. produk paling lemah
4. channel terbaik
5. hari dengan penjualan tertinggi
6. kemungkinan masalah
7. rekomendasi 7 hari ke depan
8. metrik yang perlu dipantau

Gunakan bahasa sederhana.
```

Data tidak harus rumit.

UMKM bisa mulai dari spreadsheet sederhana.

Baca juga:

- [AI untuk Produktivitas](/blog/ai-untuk-produktivitas/)
- [Tools AI untuk Bisnis](/blog/tools-ai-untuk-bisnis/)

---

# Prompt ChatGPT untuk SOP UMKM

SOP membantu bisnis kecil berjalan lebih rapi.

Gunakan prompt:

```text
Buatkan SOP sederhana untuk UMKM.

Jenis usaha:
[jelaskan]

Proses yang ingin dibuat SOP:
[order masuk / packing / pengiriman / komplain / stok / admin WhatsApp]

Buat SOP berisi:
1. tujuan
2. alat yang dibutuhkan
3. langkah kerja
4. siapa yang bertanggung jawab
5. checklist
6. kesalahan yang harus dihindari
7. standar kualitas
8. contoh template pesan jika perlu

Gunakan bahasa sederhana agar mudah dipahami tim kecil.
```

Contoh SOP yang cocok untuk UMKM:

```text
SOP membalas chat pelanggan,
SOP packing pesanan,
SOP update stok,
SOP follow-up pembayaran,
SOP menangani komplain,
SOP posting konten harian.
```

---

# Prompt ChatGPT untuk Riset Kompetitor UMKM

Riset kompetitor tidak harus rumit.

Tujuannya bukan meniru, tetapi memahami pasar.

Gunakan prompt:

```text
Bertindaklah sebagai analis kompetitor UMKM.

Saya punya usaha:
[jelaskan]

Kompetitor yang saya amati:
[tempel nama atau deskripsi kompetitor]

Data yang saya lihat:
[harga, produk, konten, promo, review, keunggulan, kelemahan]

Bantu saya menganalisis:
1. keunggulan kompetitor
2. kelemahan kompetitor
3. peluang diferensiasi
4. ide promosi yang bisa diadaptasi secara etis
5. hal yang tidak perlu ditiru
6. rekomendasi posisi bisnis saya

Batasan:
Jangan menyarankan menyalin mentah strategi kompetitor.
```

Riset kompetitor yang baik tetap memperhatikan identitas brand sendiri.

---

# Prompt ChatGPT untuk Mengelola Review dan Testimoni

Review pelanggan bisa menjadi bahan marketing.

Gunakan prompt:

```text
Saya punya beberapa testimoni pelanggan.

Testimoni:
[tempel testimoni]

Bantu saya membuat:
1. ringkasan insight dari testimoni
2. 5 caption dari testimoni
3. 5 ide konten social proof
4. highlight manfaat produk
5. kalimat promosi yang tetap natural
6. versi testimoni yang dirapikan tanpa mengubah makna

Batasan:
Jangan mengubah isi testimoni secara menyesatkan.
```

Testimoni bisa digunakan untuk:

- posting Instagram,
- story,
- website,
- marketplace,
- katalog,
- landing page,
- WhatsApp broadcast.

---

# Workflow UMKM Menggunakan ChatGPT

Berikut workflow sederhana agar ChatGPT benar-benar membantu bisnis.

```text
Tentukan masalah bisnis
→ buat prompt sesuai kebutuhan
→ hasilkan draft
→ edit sesuai kondisi bisnis
→ pakai di channel jualan
→ catat hasil
→ perbaiki prompt
```

Contoh workflow promosi:

```text
produk yang ingin dijual
→ ChatGPT buat ide promosi
→ pilih 3 ide terbaik
→ buat caption
→ buat visual
→ posting
→ catat chat masuk dan order
```

Contoh workflow customer service:

```text
kumpulkan pertanyaan pelanggan
→ ChatGPT buat FAQ
→ admin review
→ simpan template jawaban
→ pakai saat membalas chat
→ update jika ada pertanyaan baru
```

Contoh workflow laporan:

```text
input data penjualan
→ ChatGPT bantu analisis
→ catat produk terlaris
→ buat keputusan stok/promosi
→ cek lagi minggu depan
```

---

# Kesalahan Menggunakan ChatGPT untuk UMKM

## 1. Prompt Terlalu Umum

Jika prompt terlalu umum, hasilnya juga umum.

Selalu beri konteks usaha, produk, pelanggan, dan tujuan.

## 2. Semua Output Langsung Dipakai

Output ChatGPT tetap harus diedit.

Sesuaikan dengan stok, harga, promo, aturan bisnis, dan gaya bahasa brand.

## 3. Membuat Klaim Berlebihan

Hindari klaim seperti:

```text
pasti laris,
pasti viral,
terbaik nomor satu,
paling ampuh,
dijamin semua orang suka.
```

Gunakan kalimat yang lebih aman dan realistis.

## 4. Tidak Mencatat Hasil

UMKM perlu mencatat apakah promosi berhasil.

Minimal catat:

```text
konten yang diposting,
jumlah chat masuk,
jumlah order,
produk terjual,
komentar pelanggan,
promo yang dipakai.
```

## 5. Tidak Menjaga Data Pelanggan

Jangan memasukkan data pribadi pelanggan ke tools AI publik.

Hindari memasukkan:

- nomor telepon,
- alamat lengkap,
- bukti transfer,
- data identitas,
- informasi pribadi sensitif.

---

# Checklist Prompt ChatGPT untuk UMKM

Gunakan checklist ini sebelum memakai prompt.

```text
Jenis usaha sudah jelas
Produk atau jasa sudah dijelaskan
Target pelanggan sudah disebutkan
Tujuan prompt jelas
Channel jualan disebutkan
Gaya bahasa ditentukan
Format output diminta
Batasan klaim ditulis
Tidak ada data pribadi pelanggan
Output akan direview sebelum dipakai
```

Checklist ini membuat hasil ChatGPT lebih siap digunakan.

---

# Prompt Master ChatGPT untuk UMKM

Gunakan prompt master berikut jika ingin membuat strategi UMKM dari awal.

```text
Bertindaklah sebagai konsultan bisnis UMKM.

Saya memiliki usaha:

Jenis usaha:
[jelaskan]

Produk/jasa:
[jelaskan]

Target pelanggan:
[jelaskan]

Channel jualan:
[offline/WhatsApp/Instagram/TikTok/marketplace/website]

Masalah utama:
[jelaskan]

Tujuan 30 hari:
[menambah pelanggan/meningkatkan repeat order/menaikkan omzet/merapikan operasional]

Buatkan:
1. analisis masalah utama
2. strategi 30 hari
3. ide promosi mingguan
4. 30 ide konten
5. template WhatsApp pelanggan
6. ide bundling
7. SOP sederhana
8. metrik yang harus dicatat
9. checklist eksekusi harian

Batasan:
Gunakan bahasa sederhana.
Fokus pada langkah yang bisa dilakukan UMKM kecil.
Jangan membuat klaim berlebihan.
```

# Prompt Repurposing Konten UMKM

Gunakan prompt ini untuk mengubah satu ide menjadi banyak konten.

```text
Ubah satu ide konten berikut menjadi materi promosi UMKM.

Ide utama:
[tulis ide]

Jenis usaha:
[jelaskan]

Produk:
[jelaskan]

Target pelanggan:
[jelaskan]

Buat:
1. caption Instagram
2. script TikTok 30 detik
3. status WhatsApp
4. broadcast WhatsApp
5. ide carousel 5 slide
6. headline promo
7. CTA
8. versi soft selling
9. versi hard selling yang tetap sopan

Gunakan bahasa Indonesia natural.
```

# Prompt Evaluasi Promosi UMKM

Gunakan prompt ini setelah menjalankan promosi.

```text
Bertindaklah sebagai konsultan marketing UMKM.

Saya sudah menjalankan promosi.

Data promosi:
[tempel data]

Channel:
[Instagram/WhatsApp/TikTok/marketplace/offline]

Hasil:
[jumlah chat/order/penjualan/komentar]

Bantu saya:
1. menilai apakah promosi berhasil
2. mencari kemungkinan penyebab hasilnya
3. memberi saran perbaikan
4. membuat ide promosi berikutnya
5. membuat 5 caption baru
6. membuat checklist eksekusi 7 hari ke depan

Gunakan bahasa sederhana dan praktis.
```

## Jelajahi Panduan AI Lainnya

* [Pusat AI untuk Bisnis](/blog/pusat-ai-untuk-bisnis/)
* [ChatGPT untuk UMKM](/blog/chatgpt-untuk-umkm/)
* [Cara Menggunakan ChatGPT untuk UMKM](/blog/cara-menggunakan-chatgpt-untuk-umkm/)
* [Tools AI untuk UMKM](/blog/tools-ai-untuk-umkm/)
* [Studi Kasus UMKM Menggunakan AI](/blog/studi-kasus-umkm-menggunakan-ai/)
* [Strategi AI untuk UMKM Indonesia](/blog/strategi-ai-untuk-umkm-indonesia/)
* [Cara Menghasilkan Uang dengan AI](/blog/cara-menghasilkan-uang-dengan-ai/)

## Kesimpulan

Prompt yang tepat dapat membantu UMKM memanfaatkan ChatGPT secara maksimal. Mulai dari pemasaran, penjualan, pelayanan pelanggan, hingga pengembangan bisnis, AI dapat menjadi asisten yang membantu meningkatkan produktivitas dan efisiensi usaha.
